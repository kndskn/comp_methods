...
for i in range(3):
    ...
    plt.plot(t_ax, sol_imp[0], color = 'blue', label = 'x, fact='+str(10**i))
# plt.plot(t_ax, sol[1], color = 'blue', label = 'y')
plt.plot(tspace, anal_sol(tspace, 5, -3)[0], color = 'red', label = 'an_x, C1=1, C2=1')
...
for i in range(3):
    ...
    # print(np.arange(100)+1)
    plt.plot(t_ax, sol_dir[0], color = 'blue', label = 'x, fact='+str(10**i))
# plt.plot(t_ax, sol[1], color = 'blue', label = 'y')
plt.plot(tspace, anal_sol(tspace, 5, -3)[0], color = 'red', label = 'an_x, C1=1, C2=1')
plt.ylim([0.9*np.min(anal_sol(tspace, 5, -3)[0]), 1.1*np.max(anal_sol(tspace, 5, -3)[0])])
plt.legend()
...
for i in range(3):
    sol_dir = np.array(Euler(y_0, N//(10**i), dh*10**i)).transpose()
    t_ax = (np.arange(N//(10**i))*(10**i))*dh
    plt.plot(t_ax, sol_dir[0], color = 'blue', label = 'direct, fact='+str(10**i), zorder = 1) # plotting of direct Euler
    sol_imp = np.array(Euler_implicit(y_0, N//(10**i), 3, dh*10**i)).transpose()
    L = np.shape(sol_imp)[1]
    t_ax = (np.arange(L)*N/L)*dh
